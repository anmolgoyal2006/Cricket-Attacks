...
import { Users, Share2, Trophy, Loader2 } from 'lucide-react';
...
export default function ComparePage() {
  ...
  useEffect(() => {
    async function fetchPlayers() {
      try {
        const data = await cardsApi.getAll({ limit: '50', sort: 'name' });
        setPlayers(data.cards || []);
        if (data.cards?.length >= 2) {
          setPlayerA(data.cards[0]);
          setPlayerB(data.cards[1]);
        }
      } catch {
        setPlayers([]);
      } finally {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <div className="glass rounded-2xl p-6">
            ...
            <select
              value={playerA?._id || ''}
              onChange={(e) => setPlayerA(players.find(p => p._id === e.target.value) || null)}
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <select
              value={playerB?._id || ''}
              onChange={(e) => setPlayerB(players.find(p => p._id === e.target.value) || null)}
              className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 appearance-none cursor-pointer font-body text-lg"
            >
              {players.map(player => (
                <option key={player._id} value={player._id} className="bg-cricket-stadium">
