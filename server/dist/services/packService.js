...
async function openPack(packType) {
    ...
    const cardPromises = Object.entries(rarityCounts).map(([rarity, count]) => Player_1.default.aggregate([
        { $match: { rarity } },
        { $sample: { size: count } },
    ]));
    ...
    const playerCards = await Player_1.default.find({ _id: { $in: allCardDocs.map((c) => c._id) } });
