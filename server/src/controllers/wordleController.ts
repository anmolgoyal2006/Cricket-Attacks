...
export async function getDailyWordle(req: Request, res: Response, next: NextFunction) {
  try {
    const allPlayers = await Player.find({}).select('name').lean();
...
export async function submitWordleGuess(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!guess || typeof guess !== 'string') {
      throw new BadRequestError('guess is required');
    }
    if (!guessNumber || guessNumber < 1 || guessNumber > 6) {
      throw new BadRequestError('guessNumber must be 1–6');
    }
    ...
    const guessedPlayer = allFull.find(
