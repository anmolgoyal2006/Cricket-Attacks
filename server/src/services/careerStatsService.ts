...
export async function foldMatchStatsIntoCareer(matchId: string | mongoose.Types.ObjectId): Promise<void> {
  // Idempotency guard — skip if already processed
  const match = await ScoringMatch.findById(matchId);
  if (!match) return;
  if (match.statsProcessed) return;

  const allMatchStats = await PlayerMatchStats.find({ matchId });
  if (allMatchStats.length === 0) {
    // Nothing to fold — still mark processed to avoid future re-runs
    await ScoringMatch.findByIdAndUpdate(matchId, { statsProcessed: true });
    return;
  }
  ...
  for (const ms of allMatchStats) {
    ...
    const current = await PlayerCareerStats.findOne({ playerId });
 ...
 */
export async function linkGuestStatsToUser(
  userId: string | mongoose.Types.ObjectId,
  username: string
): Promise<{ linked: number }> {
  // Find all guest stats rows with this username (case-insensitive)
  const guestStats = await PlayerMatchStats.find({
    playerId: null,
    guestName: { $regex: new RegExp(`^${username}$`, 'i') },
  });

  if (guestStats.length === 0) return { linked: 0 };

  const matchIds = [...new Set(guestStats.map((s) => s.matchId.toString()))];

  // Assign real playerId to each guest stat row
  await PlayerMatchStats.updateMany(
    { playerId: null, guestName: { $regex: new RegExp(`^${username}$`, 'i') } },
    { $set: { playerId: userId, guestName: null } }
  );

  // Patch ScoringMatch player entries that still have guestName
  await ScoringMatch.updateMany(
    {
      $or: [
 ...
 */
async function foldSinglePlayerStats(
  userId: string | mongoose.Types.ObjectId,
  matchId: string | mongoose.Types.ObjectId
): Promise<void> {
  const ms = await PlayerMatchStats.findOne({ matchId, playerId: userId });
  if (!ms) return;
  ...
  const current = await PlayerCareerStats.findOne({ playerId: userId });
  const currentHighest = current?.highestScore ?? 0;
  const newHighest = bat.runs > currentHighest ? bat.runs : currentHighest;
