...
export function setupMatchmaking(
  io: Server,
  battleRooms: BattleRoomHandlers
) {
  setInterval(() => {
    ...
    if (!p1Socket?.connected || !p2Socket?.connected) {
      if (p1Socket?.connected) queue.unshift(player1);
      if (p2Socket?.connected) queue.unshift(player2);
      return;
    }
    ...
    battleRooms.initializeBattle(
      player1.socket as AuthenticatedSocket,
      io,
      battleId,
      {
...
function startBattleCountdown(io: Server, battleId: string) {
  let countdown = 3;
